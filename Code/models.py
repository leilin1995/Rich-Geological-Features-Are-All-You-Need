...
class SingleConv(nn.Sequential):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, order="cbrd", num_groups=0, padding=1,
...
class DoubleConv(nn.Sequential):
    ...
    def __init__(self, in_channels, out_channels, encoder, kernel_size=3, order="cbrd", num_groups=0, padding=1,
                 upscale=2, dropout_prob=0.1, is3d=True):
...
class ResNetBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, order="cbrd", num_groups=0, is3d=True, **kwargs):
        super(ResNetBlock, self).__init__()
        if in_channels != out_channels:
            # conv1*1 for increasing the number of channels
            if is3d:
...
class ResNetBlockSE(ResNetBlock):
    def __init__(self, in_channels, out_channels, kernel_size=3, order="cbrd", num_groups=8, se_module='scse',
                 **kwargs):
...
class Encoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv_kernel_size=3, apply_pooling=True, pool_kernel_size=2,
                 pool_type="max", basic_module=DoubleConv, conv_layer_order="cbrd", num_groups=0, padding=1, upscale=2,
                 dropout_prob=0.1, is3d=True):
...
class Decoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv_kernel_size=3, scale_factor=2, basic_module=DoubleConv,
                 conv_layer_order='cbrd', num_groups=8, padding=1, upsample='deconv',
                 dropout_prob=0.1, is3d=True):
...
class AbstractUNet(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid, basic_module, f_maps=64, layer_order='cbrd',
                 num_groups=8, num_levels=4, is_segmentation=True, conv_kernel_size=3, pool_kernel_size=2,
                 conv_padding=1, conv_upscale=2, upsample='deconv', dropout_prob=0.1, is3d=True):
...
class UNet3D(AbstractUNet):
    ...
    def __init__(self, in_channels, out_channels, final_sigmoid=True, f_maps=64, layer_order='gcr',
                 ...
                 conv_upscale=2, upsample='deconv', dropout_prob=0.1, **kwargs):
...
def build_decoder(num_classes, backbone, BatchNorm):
    return DeepLabDecoder(num_classes, backbone, BatchNorm)
...
def ResNet101(output_stride, BatchNorm):
    model = DeepLabResNet(Bottleneck, [3, 4, 23, 3], output_stride, BatchNorm)
    return model


def ResNet50(output_stride, BatchNorm):
    model = DeepLabResNet(Bottleneck, [3, 4, 6, 3], output_stride, BatchNorm)
    return model


def build_resnet(output_stride, BatchNorm, res_model="ResNet101"):
    if res_model == "ResNet101":
...
class Deeplab(nn.Module):
    def __init__(self, output_stride=16, num_classes=2,
                 sync_bn=True, freeze_bn=False, is_segmentation=True, final_sigmoid=False):
